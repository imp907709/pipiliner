...
Host ports use **+1000 offset** so satellite infra does not bind 5432 / 5672 / 9000 on localhost. See [DOCKER.md](DOCKER.md).

| Service | Host URL |
|---------|----------|
| PostgreSQL | `localhost:6432` |
| RabbitMQ AMQP | `localhost:6672` |
| RabbitMQ UI | http://localhost:16672 |
| MinIO API | http://localhost:9100 |
| MinIO console | http://localhost:9101 |
| API (Docker) | http://localhost:6180 |
| API (dotnet run) | http://localhost:5080 |

```bash
# 1. Infrastructure (PostgreSQL, RabbitMQ, MinIO)
docker compose -f docker-compose.infra.yml up -d

# 2. API + Worker (optional)
docker compose up -d --build

# Local debug: ASPNETCORE_ENVIRONMENT=Development / DOTNET_ENVIRONMENT=Development
# uses appsettings.Development.json → host ports above
